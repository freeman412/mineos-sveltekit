<script lang="ts">
	...
	import { extractMinecraftVersion, getCompatibilityBadge } from '$lib/utils/version';
	import type {
	...
	function scheduleSearch() {
		...
		searchDebounce = setTimeout(() => {
			searchResourcePacks(true);
		}, 350);
	}

	async function searchResourcePacks(reset = false) {
		...
		try {
			...
			const res = await fetch(
				`/api/servers/${encodeURIComponent(serverName)}/resourcepacks/modrinth/search?${params.toString()}`
			);
	...
	async function showDetail(projectId: string) {
		...
		try {
			const res = await fetch(
				`/api/servers/${encodeURIComponent(serverName)}/resourcepacks/modrinth/project/${encodeURIComponent(projectId)}`
			);
			if (!res.ok) {
				const payload = await res.json().catch(() => ({}));
		...
		try {
			...
			const res = await fetch(
				`/api/servers/${encodeURIComponent(
					serverName
				)}/resourcepacks/modrinth/project/${encodeURIComponent(projectId)}/versions?${params.toString()}`
			);
			if (res.ok) {
	...
	async function installVersion(versionId: string) {
		...
		try {
			const res = await fetch(`/api/servers/${encodeURIComponent(serverName)}/resourcepacks/modrinth/install`, {
				method: 'POST',
			...
			if (!res.ok) {
				const payload = await res.json().catch(() => ({}));
				await modal.error(payload.error || 'Failed to install resource pack');
				return;
			}

			await modal.success('Resource pack downloaded successfully.');
			detailOpen = false;
			onInstallComplete?.();
		} catch (err) {
			await modal.error(err instanceof Error ? err.message : 'Failed to install resource pack');
		} finally {
...
<div class="search-container">
	...
	{:else if searchResults.length > 0}
		...
		{:else if hasMore}
			<button class="load-more-btn" onclick={() => searchResourcePacks(false)}>
				Load More
			</button>
		{/if}
