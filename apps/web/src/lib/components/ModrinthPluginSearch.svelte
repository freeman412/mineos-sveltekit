<script lang="ts">
	...
	async function searchPlugins(reset = false) {
		...
		try {
			...
			const res = await fetch(
				`/api/servers/${encodeURIComponent(serverName)}/plugins/modrinth/search?${params.toString()}`
			);
	...
	async function showDetail(projectId: string) {
		...
		try {
			const res = await fetch(
				`/api/servers/${encodeURIComponent(serverName)}/plugins/modrinth/project/${encodeURIComponent(projectId)}`
			);
			if (!res.ok) {
				const payload = await res.json().catch(() => ({}));
		...
		try {
			...
			const res = await fetch(
				`/api/servers/${encodeURIComponent(
					serverName
				)}/plugins/modrinth/project/${encodeURIComponent(projectId)}/versions?${params.toString()}`
			);
			if (res.ok) {
	...
	async function installVersion(versionId: string) {
		...
		try {
			const res = await fetch(`/api/servers/${encodeURIComponent(serverName)}/plugins/modrinth/install`, {
				method: 'POST',
			...
			if (!res.ok) {
				const payload = await res.json().catch(() => ({}));
				await modal.error(payload.error || 'Failed to install plugin');
				return;
			}

			await modal.success('Plugin downloaded. Restart the server to apply changes.');
			detailOpen = false;
			onInstallComplete?.();
		} catch (err) {
			await modal.error(err instanceof Error ? err.message : 'Failed to install plugin');
		} finally {
...
<div class="search-container">
	...
	{:else if searchResults.length > 0}
		...
		{:else if hasMore}
			<button class="load-more-btn" onclick={() => searchPlugins(false)}>
				Load More
			</button>
		{/if}
