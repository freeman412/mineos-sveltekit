...
export const GET: RequestHandler = async ({ params, cookies, url }) => {
	...
	const streamingUrl = `${baseUrl}/api/v1/servers/${params.name}/performance/streaming${url.search}`;
	let response = await fetch(streamingUrl, { headers });

	if (response.status === 404) {
		response.body?.cancel();
		const fallbackUrl = `${baseUrl}/api/v1/servers/${params.name}/performance/stream${url.search}`;
